...
describe('remix url helpers', () => {
  it('host strips protocol and trailing slash', () => {
    // Default (no VITE_PUBLIC_URL in test env) falls back to https://shotpolish.app
    expect(remixHost()).toBe('shotpolish.app')
  })

  it('builds a per-template remix url', () => {
    expect(buildRemixUrl('launch-indigo')).toBe('shotpolish.app/r/launch-indigo')
  })

  it('falls back to the bare host when there is no template', () => {
    expect(buildRemixUrl()).toBe('shotpolish.app')
  })
